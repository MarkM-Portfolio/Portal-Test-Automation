...
fileToSearch=$1
logLocation="/home/centos/combined_script_rendering_performance_results.log"
...
for i in "${!searchResultArray[@]}"; do
    ...
    if [[ ${searchResultArray[$i]} == *"Total\"" ]]; then
        ...
        index=$(($i + 1))
        echo "Total Samples : - $(printf '%.*d\n' 3 "${searchResultArray[$index]}")"
        echo "Total Samples : - $(echo "${searchResultArray[$index]}" | bc -l | xargs printf "%.*d\n" 3 ) "
        printf "$(echo "${searchResultArray[$index]}" | bc -l | xargs printf "%.*d\n" 3 )," >> "${logLocation}"

        index=$(($i + 3))
        # Scientific notation number
        errorPercentInScientificNumber="${searchResultArray[$index]}"
        echo "Error Percent in Scientific number: $errorPercentInScientificNumber"
        # Convert scientific notation to decimal
        errorPercentInDecimalNumber=$(printf "%f" "$errorPercentInScientificNumber")
        # Print the decimal number
        echo "Error Percent in Decimal number: $errorPercentInDecimalNumber"

        echo "Error % : - $(printf '%.*f\n' 4 "${errorPercentInDecimalNumber}") %"
        echo "Error % : - $(echo "${errorPercentInDecimalNumber}" | bc -l | xargs printf "%.*f\n" 4 ) "
        printf "$(echo "${errorPercentInDecimalNumber}" | bc -l | xargs printf "%.*f\n" 4 )," >> "${logLocation}"
    fi
done
