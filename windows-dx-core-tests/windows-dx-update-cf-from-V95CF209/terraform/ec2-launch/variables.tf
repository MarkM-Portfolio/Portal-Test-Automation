...
terraform {
  backend "s3" {
    bucket = "dx-testarea"
    key    = "terraform-status/windows-dx-core-tests/windows-dx-update-cf-from-V95CF209/windows-dx-update-cf-from-V95CF209-local.key"
    region = "us-east-1"
  }
}
