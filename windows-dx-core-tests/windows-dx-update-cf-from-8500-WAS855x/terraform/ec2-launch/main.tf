# Create the AWS AMI
resource "aws_instance" "windows-dx-update-cf-from-8500-WAS855x" {
  ami                         = data.aws_ami.windows8500WAS855xbase.id
...
resource "aws_route53_record" "windows-dx-update-cf-from-8500-WAS855x" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.windows-dx-update-cf-from-8500-WAS855x.private_ip}"]
}
