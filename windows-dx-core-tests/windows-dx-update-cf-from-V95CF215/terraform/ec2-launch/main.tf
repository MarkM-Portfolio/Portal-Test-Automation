# Create the AWS AMI
resource "aws_instance" "windows-dx-update-cf-from-V95CF215" {
  ami                         = data.aws_ami.windowsV95CF215.id
...
resource "aws_route53_record" "windows-dx-update-cf-from-V95CF215" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.windows-dx-update-cf-from-V95CF215.private_ip}"]
}
