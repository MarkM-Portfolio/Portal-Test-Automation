...
terraform {
  backend "s3" {
    bucket = "dx-testarea"
    key    = "terraform-status/windows-dx-core-tests/windows-dx-update-cf-from-V95CF215/windows-dx-update-cf-from-V95CF215-local.key"
    region = "us-east-1"
  }
}
