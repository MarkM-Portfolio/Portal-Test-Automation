# windows-dx-update-cf-from-V95CF207 Job

## Description

THIS IS A NEW FILE FOR WINDOWS DX UPDATE CF - FROM V95 CF207
CHANGES HAVE BEEN MADE TO THIS FILE TO ACCOUNT FOR WINDOWS-SPECIFIC INFORMATION.

This pipeline takes a configured Windows V95 CF207 AMI and performs an update to the configured CF version.
Therefore an EC2 instance gets created based on the Windows V95 CF207 AMI. This EC2 instance will be configured with a Route53 record.
...
The running execution of the pipeline will be labeled in the following pattern: `windows-dx-update-cf-from-V95CF207_YYYYMMDD-hhmm`.

The pipeline will create an EC2 instance based on the configured Windows V95 CF207 AMI.
This instance name will be the name of the current pipeline execution, so `windows-dx-update-cf-from-V95CF207_YYYYMMDD-hhmm`.
...
After creation of the EC2 instance, a Route53 record for the instance with the address `windows-dx-update-cf-from-V95CF207_YYYYMMDD-hhmm.team-q-dev.com` will be created.
