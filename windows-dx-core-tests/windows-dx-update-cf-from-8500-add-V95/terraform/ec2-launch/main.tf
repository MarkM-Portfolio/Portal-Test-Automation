# Create the AWS AMI
resource "aws_instance" "windows-dx-update-cf-from-8500-add-V95" {
  ami                         = data.aws_ami.windows-dx-update-cf-from-8500.id
  ...
  tags = {
    ...
    build           = data.aws_ami.windows-dx-update-cf-from-8500.tags.build
    baseImageName   = data.aws_ami.windows-dx-update-cf-from-8500.tags.Name
  }
...
resource "aws_route53_record" "windows-dx-update-cf-from-8500-add-V95" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.windows-dx-update-cf-from-8500-add-V95.private_ip}"]
}
