...
terraform {
  backend "s3" {
    bucket = "dx-testarea"
    key    = "terraform-status/windows-dx-core-tests/windows-dx-update-cf-from-cf17-sql/windows-dx-update-cf-from-cf17-sql-local.key"
    region = "us-east-1"
  }
}
