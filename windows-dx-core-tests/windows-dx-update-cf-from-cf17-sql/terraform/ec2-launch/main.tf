# Create the AWS AMI
resource "aws_instance" "windows-dx-update-cf-from-cf17-sql" {
  ami                         = data.aws_ami.windowsV95CF17SQL.id
...
resource "aws_route53_record" "windows-dx-update-cf-from-cf17-sql" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.windows-dx-update-cf-from-cf17-sql.private_ip}"]
}
