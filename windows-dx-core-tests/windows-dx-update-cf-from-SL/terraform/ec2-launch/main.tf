# Create the AWS AMI
resource "aws_instance" "windows-dx-update-cf-from-SL" {
  ami                         = data.aws_ami.windowsSL.id
...
resource "aws_route53_record" "windows-dx-update-cf-from-SL" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.windows-dx-update-cf-from-SL.private_ip}"]
}
