# Create the AWS AMI
resource "aws_instance" "dx-cf-art-tests" {
  ami                         = data.aws_ami.dx-update-cf-oracle.id
  ...
  tags = {
    ...
    build           = data.aws_ami.dx-update-cf-oracle.tags.build
    baseImageName   = data.aws_ami.dx-update-cf-oracle.tags.Name
  }
