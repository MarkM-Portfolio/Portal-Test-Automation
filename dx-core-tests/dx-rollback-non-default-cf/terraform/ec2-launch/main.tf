# Create the AWS AMI
resource "aws_instance" "dx-rollback-non-default-cf" {
  ami                         = data.aws_ami.dx-update-non-default-cf.id
  ...
  tags = {
    ...
    build           = data.aws_ami.dx-update-non-default-cf.tags.build
    baseImageName   = data.aws_ami.dx-update-non-default-cf.tags.Name
  }
