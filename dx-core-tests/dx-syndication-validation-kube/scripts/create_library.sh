...
# Login
echo "POST Login"
curl --cookie-jar cookie.txt -s -o response.txt -w "%{http_code}" --location --request POST "$1/portal/cxml/04_SD9ePMtCP1I800I_KydQvyHFUBADPmuQy?userid=wpsadmin&password=wpsadmin" \
...
curl -b cookie.txt -s  -o SyndicationLibrary.txt -w "%{http_code}" --location --request POST "$1/mycontenthandler/wcmrest/Library?mime-type=application/json" \
