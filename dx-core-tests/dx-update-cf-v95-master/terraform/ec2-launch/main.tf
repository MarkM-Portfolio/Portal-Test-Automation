# Create the AWS AMI
resource "aws_instance" "dx-update-cf-v95-master" {
...
resource "aws_route53_record" "dx-update-cf-v95-master" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.dx-update-cf-v95-master.private_ip}"]
}
