...
This pipeline executes DX Syndication validation tests.

# Steps

1. Creates two or three new EC2 instances of the latest on-prem CF build
2. Enables content sites on each instance 
3. Deploys cross-reference libraries on the Syndicator on-premise instance.
4. Enables syndication relationship between the two on-premise systems
5. Creates new library and populate with content and new JCR types
6. Verifies that the contents of cross-reference library and new JCR types are successfully syndicated.

# Jenkins Pipeline

https://portal-jenkins-test.cwp.pnp-hcl.com/job/CI/job/dx-core-tests/job/dx-syndication-validation/

# Pipeline Parameters

| Parameter  | Description |
| ------------- | ------------- |
| DXBuildNumber  | Install Portal 9.5 CF build |
| [THREE_WAY_SYNDICATION](#syndication-between-3-instances) | Enable syndication between 3 instances  |
| [SYNDICATE_LIBRARIES](#define-which-libraries-to-syndicate)  | Library names to Syndicate for eg. Web Content |
| [SYNDICATE_ITEMS](#define-which-items-in-the-library-to-syndicate)  | Select which items from the library to syndicate.  |
