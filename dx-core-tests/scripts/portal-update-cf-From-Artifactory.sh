...
# Download CF package from Artifactory and untar
SERVER_ZIP_URL="https://artifactory.cwp.pnp-hcl.com/artifactory/quintana-generic/dx-build-output/core/$BUILD_LABEL/inst_repo.tar"
echo "Downloading CF package from Artifactory - $SERVER_ZIP_URL"
curl -O $SERVER_ZIP_URL
echo "Untarring the CF package"
tar -xvf inst_repo.tar
echo "Preparing to use the zip from the untarred package"
cp buildartifacts/iim.server/WP8500* .
rm -rf props.properties buildnum.txt inst_repo.tar buildartifacts/
unzip WP8500*
echo "CF package is ready for applyCF"
