...
echo "AdminTask.renewCertificate('[-keyStoreName NodeDefaultKeyStore -keyStoreScope (cell):ci-linuxstal-39sht7rxCell:(node):ci-linuxstal-39sht7rxNode -certificateAlias default ]')
AdminConfig.save()" > renewCert.py
./wsadmin.sh -user wpsadmin -password $PORTAL_ADMIN_PWD -lang jython -f renewCert.py
./stopServer.sh WebSphere_Portal -username wpsadmin -password $PORTAL_ADMIN_PWD
...
#Add to required values to wkplc.properties before applying CF
echo "Adding required values to wkplc.properties"
echo "config.wizard.profile.name=$PORTAL_ADMIN_PWD" >> /opt/IBM/WebSphere/wp_profile/ConfigEngine/properties/wkplc.properties
echo "config.wizard.profile.password=$PORTAL_ADMIN_PWD" >> /opt/IBM/WebSphere/wp_profile/ConfigEngine/properties/wkplc.properties

#Apply CF
echo "Starting to Apply CF"
cd /opt/IBM/WebSphere/wp_profile/PortalServer/bin
install_95=true ./applyCF.sh -DPortalAdminPwd=$PORTAL_ADMIN_PWD -DWasPassword=$PORTAL_ADMIN_PWD
...
if grep -q Portal95=true /opt/IBM/WebSphere/PortalServer/wps.properties
then 
