# Create the AWS AMI
resource "aws_instance" "dx-update-cf-ora21c" {
  ami                         = data.aws_ami.cf19base.id
...
resource "aws_route53_record" "dx-update-cf-ora21c" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.dx-update-cf-ora21c.private_ip}"]
}
