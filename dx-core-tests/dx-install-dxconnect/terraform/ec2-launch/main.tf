# Create the AWS AMI
resource "aws_instance" "dx-install-dxconnect" {
  ami                         = data.aws_ami.dx-update-cf.id
  ...
  tags = {
    ...
    build           = data.aws_ami.dx-update-cf.tags.build
    baseImageName   = data.aws_ami.dx-update-cf.tags.Name
  }
...
resource "aws_route53_record" "dx-install-dxconnect" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.dx-install-dxconnect.private_ip}"]
}
