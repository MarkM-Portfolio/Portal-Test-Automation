# dx-update-cf-from-8500-WAS855x Job

## Description

This pipeline takes a configured 8500 with WAS 855x Linux base AMI and performs an update to the configured CF version.
Therefore an EC2 instance gets created based on the 8500 with WAS 855x Linux base AMI. This EC2 instance will be configured with a Route53 record.
...
The running execution of the pipeline will be labeled in the following pattern: `dx-update-cf-from-8500-WAS855x_YYYYMMDD-hhmm`.

The pipeline will create an EC2 instance based on the configured 8500 with WAS 855x Linux base AMI.
This instance name will be the name of the current pipeline execution, so `dx-update-cf-from-8500-WAS855x_YYYYMMDD-hhmm`.
...
After creation of the EC2 instance, a Route53 record for the instance with the address `dx-update-cf-from-8500-WAS855x_YYYYMMDD-hhmm.team-q-dev.com` will be created.
...
Housekeeping (separate job) will check those tags and remove expired resources.
