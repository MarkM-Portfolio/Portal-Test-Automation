# Create the AWS AMI
resource "aws_instance" "dx-update-non-default-cf" {
  ami                         = data.aws_ami.portal85.id
...
resource "aws_route53_record" "dx-update-non-default-cf" {
  zone_id = var.HOSTED_ZONE
  name    = var.TEST_RUN_ID
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.dx-update-non-default-cf.private_ip}"]
}
