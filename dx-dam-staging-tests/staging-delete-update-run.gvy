...
def uploadAssets(SERVER_HOST_PUBLISHER, terraformVarsEC2, pipelineParameters){
        configFileProvider([configFile(fileId: 'test-automation-deployments', variable: 'DEPLOY_KEY')
                ]) {
                    script {
                        ...
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                echo 'DAM upload of 36 colkections with 30 assets each'
...
def checkForDAMOperations(kubeDeploymentHostName, pipelineParameters){
            ...
            configFileProvider([configFile(fileId: 'test-automation-deployments', variable: 'DEPLOY_KEY')
                        ...
                        script {
                            try {
                                ...
                                sh """
                                    scp -v -i test-automation-deployments.pem -o StrictHostKeyChecking=no ./capture-dam-operations-time.sh centos@${kubeDeploymentHostName}:/home/centos/native-kube/
                                """

                                /* Run bash script to capture operations time */
                                sh  """
                                    ssh -i test-automation-deployments.pem -o StrictHostKeyChecking=no centos@${kubeDeploymentHostName} \
                                ...
                                sh """
                                    ...
                                    cp $DEPLOY_KEY test-automation-deployments.pem
                                    chmod 0600 test-automation-deployments.pem
                                    ssh -i test-automation-deployments.pem -o StrictHostKeyChecking=no centos@${kubeDeploymentHostName} 'printf "Failed," >> /home/centos/native-kube/dam_performance_results.log'
                                """
                            }
                        }
                    }
...
def checkSubscriberSyncStatus(SERVER_HOST_PUBLISHER, SERVER_HOST_SUBSCRIBER, terraformVarsEC2, pipelineParameters){
        configFileProvider([
    ...
    stages {
        stage('Load modules and configuration') {
            steps {
                script {
                    ...
                    dxParametersLoadFromFile(pipelineParameters, "${env.WORKSPACE}/dx-dam-staging-tests/delete-update-run-parameters.yaml")
                }
            }
        }
            ...
            steps {
                ...
                dir("${WORKSPACE}/dx-dam-staging-tests") {
                    script {
                        ...
                        SERVER_HOST_PUBLISHER = "${pipelineParameters.INSTANCE_NAME}-sync-publisher${pipelineParameters.DOMAIN_SUFFIX}"
                        SERVER_HOST_SUBSCRIBER = "${pipelineParameters.INSTANCE_NAME}-sync-subscriber${pipelineParameters.DOMAIN_SUFFIX}"

                        echo 'Assigning hostname + timestamp'
                        terraformVarsEC2.tfvar_instance_name = "DAM-Staging-delete-update-run_${dateFormat.format(date)}"
        ...
        stage('Undeploying the publisher application in k8 environment') {
            steps {
                script {
                    buildParameters = []
                    buildParameters.add(string(name: 'INSTANCE_NAME', value: "${pipelineParameters.INSTANCE_NAME}-sync-publisher"))
        ...
        stage('Undeploying the subscriber application in k8 environment') {
            steps {
                script {
                    buildParameters = []
                    buildParameters.add(string(name: 'INSTANCE_NAME', value: "${pipelineParameters.INSTANCE_NAME}-sync-subscriber"))
                    buildParameters.add(string(name: 'KUBE_FLAVOUR', value: pipelineParameters.KUBE_FLAVOUR))
                    build(job: "${pipelineParameters.KUBE_UNDEPLOY_JOB}",
                          parameters: buildParameters,
                          propagate: true,
         ...
         */
        stage('Run JMeter tests to upload assets for delete') {
            steps {
                uploadAssets(SERVER_HOST_PUBLISHER, terraformVarsEC2, pipelineParameters)
                script{
        ...
        /*
         * Once JMeter scripts are executed then we will run the below stage to capture operations time for syndicated assets in publisher.
         */
        stage('Run scripts to capture operations time in publisher environment') {
            steps {
                checkForDAMOperations(SERVER_HOST_PUBLISHER, pipelineParameters)
            }
        }
        ...
        stage('Run scripts to capture operations time in subscriber environment') {
            steps {
                checkForDAMOperations(SERVER_HOST_SUBSCRIBER, pipelineParameters)
                script{
        ...
        /*
         * Once subscriber is created, we need to run the JMeter scripts to delete collections in publisher environment.
         */
        stage('Run JMeter tests to delete collections') {
            steps {
                configFileProvider([
                    configFile(fileId: 'test-automation-deployments', variable: 'DEPLOY_KEY')
                ]) {
                    script {
                        ...
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                echo 'Delete DAM first 30 collections'
                                // capture delete start time
                                deleteStartTime = new Date()

                                sh """
                                    chmod 600 ${DEPLOY_KEY}
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && chmod 400 ./keys/Performance-Test-Keypair.pem'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && scp -r -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem jmeter/DAM_Staging_to_Production_Tests/* centos@performance-test-jmeter-master.team-q-dev.com:~/data/dam/staging-to-prod'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && ssh -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem centos@performance-test-jmeter-master.team-q-dev.com \'env JVM_ARGS="-Dnashorn.args=--no-deprecation-warning" jmeter -JProtocol=${pipelineParameters.SERVER_PROTOCOL} -JHost=${SERVER_HOST_PUBLISHER} -JPort=${pipelineParameters.SERVER_PORT} -JRingAPIPort='' -JDAMPort='' -JDocumentsPath=/home/centos/data/dam/DAMRegression/Dataset/documents -JImagesPath=/home/centos/data/dam/DAMRegression/Dataset/images -JVideosPath=/home/centos/data/dam/DAMRegression/Dataset/videos -JCSVPath=/home/centos/data/dam/DAMRegression/Dataset/dam_users.csv -j jmeter.save.saveservice.output_format=xml -n -t /home/centos/data/dam/staging-to-prod/${pipelineParameters.DELETE_JMX_FILE} -l /home/centos/test_logs/test_log_dam_staging_upload_assets_${nowDate.format(date)}.jtl\''
                                """
        ...
        /*
         * Once JMeter scripts are executed then we will run the below stage to capture operations time for deleted assets in publisher.
         */
        stage('Run scripts to capture operations time in publisher environment after assets deleted') {
            steps {
                checkForDAMOperations(SERVER_HOST_PUBLISHER, pipelineParameters)
            }
        }
         ...
         * Once JMeter scripts are executed then we will run the below stage to capture operations time for syndicated assets in subscriber.
         */
        stage('Run scripts to capture operations time in subscriber environment - for delete assets') {
            steps {
                 checkForDAMOperations(SERVER_HOST_SUBSCRIBER, pipelineParameters)
                 script{
                    def deleteEndTime = new Date()
                    TimeDuration deleteTimeDuration = TimeCategory.minus( deleteEndTime, deleteStartTime )
                    println("DAM Staging delete time durationin HH:mm:ss - " + new Date(deleteTimeDuration.toMilliseconds()).format("HH:mm:ss"))
                 }
            }
        }
         ...
         * Once subscriber is created, we need to run the JMeter scripts to upload assets in publisher environment.
         */
        stage('Run JMeter tests to upload assets for update') {
            steps {
                uploadAssets(SERVER_HOST_PUBLISHER, terraformVarsEC2, pipelineParameters)
            }
        }
         ...
         * Once JMeter scripts are executed then we will run the below stage to capture operations time for syndicated assets in publisher.
         */
        stage('Run scripts to capture operations time in publisher environment - for update assets') {
            steps {
                checkForDAMOperations(SERVER_HOST_PUBLISHER, pipelineParameters)
            }
        }
        ...
        /*
         * Once subscriber is created, we need to run the JMeter scripts to update collections and assets in publisher environment.
         */
        stage('Run JMeter tests to update collections name and assets title') {
            steps {
                configFileProvider([
                    configFile(fileId: 'test-automation-deployments', variable: 'DEPLOY_KEY')
                ]) {
                    script {
                        ...
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                echo 'Update DAM 30 collection names and 900 asset titles'
                                ...
                                sh """
                                    ...
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && chmod 400 ./keys/Performance-Test-Keypair.pem'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && scp -r -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem jmeter/DAM_Staging_to_Production_Tests/* centos@performance-test-jmeter-master.team-q-dev.com:~/data/dam/staging-to-prod'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && ssh -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem centos@performance-test-jmeter-master.team-q-dev.com \'env JVM_ARGS="-Dnashorn.args=--no-deprecation-warning" jmeter -JProtocol=${pipelineParameters.SERVER_PROTOCOL} -JHost=${SERVER_HOST_PUBLISHER} -JPort=${pipelineParameters.SERVER_PORT} -JRingAPIPort='' -JDAMPort='' -JDocumentsPath=/home/centos/data/dam/DAMRegression/Dataset/documents -JImagesPath=/home/centos/data/dam/DAMRegression/Dataset/images -JVideosPath=/home/centos/data/dam/DAMRegression/Dataset/videos -JCSVPath=/home/centos/data/dam/DAMRegression/Dataset/dam_users.csv -j jmeter.save.saveservice.output_format=xml -n -t /home/centos/data/dam/staging-to-prod/${pipelineParameters.UPDATE_JMX_FILE} -l /home/centos/test_logs/test_log_dam_staging_upload_assets_${nowDate.format(date)}.jtl\''
                                """
                                jmeter_error = sh(script:"ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && ssh -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem centos@performance-test-jmeter-master.team-q-dev.com \'grep -o -i Exception /home/centos/test_logs/test_log_dam_staging_upload_assets_${nowDate.format(date)}.jtl | wc -l\''", returnStdout: true).trim() as Integer
                                if ("${jmeter_error}" > 0) {
                                    sh 'exit 1'
        ...
        /*
         * Once JMeter scripts are executed then we will run the below stage to capture operations time for updated assets in publisher.
         */
        stage('Run scripts to capture operations time in publisher environment after updating assets') {
            steps {
                checkForDAMOperations(SERVER_HOST_PUBLISHER, pipelineParameters)
            }
        }
         ...
         * Once JMeter scripts are executed then we will run the below stage to capture operations time for syndicated assets in subscriber.
         */
        stage('Run scripts to capture operations time in subscriber environment - for update assets') {
            steps {
                checkForDAMOperations(SERVER_HOST_SUBSCRIBER, pipelineParameters)
                script{
                    def updateEndTime = new Date()
                    TimeDuration updateTimeDuration = TimeCategory.minus( updateEndTime, updateStartTime )
                    println("DAM Staging update time duration in HH:mm:ss - " + new Date(updateTimeDuration.toMilliseconds()).format("HH:mm:ss"))
                }
            }
        }
         ...
         */
        stage('Run JMeter tests to validate collections and assets in subscriber environment') {
            options {
                timeout(time: 90, unit: 'MINUTES')
            }
            steps {
