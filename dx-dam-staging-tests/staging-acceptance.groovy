...
def cleanupInstances() {
    withCredentials([
        usernamePassword(credentialsId: "aws_credentials", passwordVariable: 'AWS_SECRET_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')
                ...
                dir("${workspace}/dx-dam-staging-tests/terraform/ec2-launch") {
                    sh(script: """
...
pipeline {
    ...
    stages {
            ...
            steps {
                ...
                dir("${WORKSPACE}/dx-dam-staging-tests") {
                    script {
                        ...
                        if (!env.VERSION){
                            env.VERSION = 'dx-dam-staging-tests'
                        }

                        if (!env.TEST_COMMAND){
                            env.TEST_COMMAND = 'staging-test-acceptance-endpoint'
                        }

                        echo "Assigning hostname + timestamp"

                        env.ENV_HOSTNAME = "Staging_Tests_${dateFormat.format(date)}"
                          ...
                          configFile(fileId: 'test-automation-deployments', variable: 'DEPLOY_KEY')
                            ...
                            dir("${workspace}/dx-dam-staging-tests/terraform/ec2-launch") {
                                ...
                                sh(script: """
                            	    ...
                            	    sh ${workspace}/dx-dam-staging-tests/scripts/wait_for_instance.sh
                                """)
                            }
                        }
                    }
                  ...
                  withCredentials([
                    usernamePassword(credentialsId: 'artifactory', passwordVariable: 'ARTIFACTORY_PASSWORD', usernameVariable: 'ARTIFACTORY_USER')
                      ...
                      configFileProvider([
                          ...
                          sh """
                            chmod 600 ${DEPLOY_KEY}
                            scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${workspace}/dx-dam-staging-tests/scripts/install-prereqs.sh centos@${env.INSTANCE_IP}:/tmp
                ...
                configFileProvider([
                    ...
                    sshagent(credentials: ['jenkins-git']) {
                        script {
                            ...
                            """
                            if(env.TEST_COMMAND == "staging-test-acceptance-endpoint"){
                                script {
                                    sh """
                                        chmod 600 ${DEPLOY_KEY}
                                        scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${workspace}/dx-dam-staging-tests/config/media-library/docker-compose.yaml centos@${env.INSTANCE_IP}:/opt/media-library
                                        scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${workspace}/dx-dam-staging-tests/scripts/pull-and-run-postgres.sh centos@${env.INSTANCE_IP}:/tmp
         ...
         */ 
        stage('Run DAM SERVER staging tests') {
            options {
                    ...
                    script {
                        ...
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                ...
                                echo "Host name will be: ${env.HOSTNAME}"
                                sh """
                                    chmod 600 ${DEPLOY_KEY}
