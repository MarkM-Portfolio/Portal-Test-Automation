...
pipeline {
    ...
    stages {
        stage('Load modules and configuration') {
            steps {
                script {
                    ...
                    dxParametersLoadFromFile(pipelineParameters, "${env.WORKSPACE}/dx-dam-staging-tests/parameters-test.yaml")
                }
            }
        }
            ...
            steps {
                ...
                dir("${WORKSPACE}/dx-dam-staging-tests") {
                    script {
                        ...
                        def date = new Date()
                        // env.ENV_HOSTNAME = "Staging_JMeter_Tests_${dateFormat.format(date)}"
                        ...
                        terraformVarsEC2.tfvar_instance_name = "DAM-Staging_${dateFormat.format(date)}"
        ...
        stage('Running staging acceptance tests in k8 environment') {
            steps {
                script {
                    buildParameters = []
                    buildParameters.add(string(name: 'EXP_API', value: "https://halo-halo-latest.team-q-dev.com:443/dx/api/core/v1"))
                    buildParameters.add(string(name: 'DAM_API', value: "https://halo-halo-latest.team-q-dev.com:443/dx/api/dam/v1"))
                    buildParameters.add(string(name: 'TARGET_BRANCH', value: "develop"))
                    buildParameters.add(booleanParam(name: 'SSL_ENABLED', value: true))
                    build(job: "${pipelineParameters.STAGING_ACCEPTANCE_TEST_JOB}", 
                        ...
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                ...
                                sh """
                                    ...
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && chmod 400 ./keys/Performance-Test-Keypair.pem'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && scp -r -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem jmeter/DAM_Staging_to_Production_Tests/* centos@performance-test-jmeter-master.team-q-dev.com:~/data/dam/staging-to-prod'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && ssh -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem centos@performance-test-jmeter-master.team-q-dev.com \'env JVM_ARGS="-Dnashorn.args=--no-deprecation-warning" jmeter -JProtocol=${pipelineParameters.SERVER_PROTOCOL} -JHost=${SERVER_HOST_PUBLISHER} -JPort=${pipelineParameters.SERVER_PORT} -JRingAPIPort='' -JDAMPort='' -JImagePath=${pipelineParameters.SERVER_IMAGEPATH} -JCSVPath=${pipelineParameters.SERVER_CSVPATH} -j jmeter.save.saveservice.output_format=xml -n -t /home/centos/data/dam/staging-to-prod/${pipelineParameters.UPLOAD_JMX_FILE} -l /home/centos/test_logs/test_log_dam_staging_upload_assets_${nowDate.format(date)}.jtl\''
                                """
        ...
        stage('Run JMeter tests to find subscriber sync status') {
            ...
            steps {
                configFileProvider([
                    ...
                    script {
                        //Waiting time for assets to be in sync before checking status.
                        sleep 300
                        def nowDate = new SimpleDateFormat("M-d-Y-HHmmss")
                        def date = new Date()
                        Exception caughtException = null;
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                ...
                                sh """
                                    ...
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && chmod 400 ./keys/Performance-Test-Keypair.pem'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && scp -r -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem jmeter/DAM_Staging_to_Production_Tests/* centos@performance-test-jmeter-master.team-q-dev.com:~/data/dam/staging-to-prod'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && ssh -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem centos@performance-test-jmeter-master.team-q-dev.com \'env JVM_ARGS="-Dnashorn.args=--no-deprecation-warning" jmeter -JProtocol=${pipelineParameters.SERVER_PROTOCOL} -JHost=${SERVER_HOST_PUBLISHER} -JPort=${pipelineParameters.SERVER_PORT} -JRingAPIPort='' -JDAMPort='' -JImagePath=${pipelineParameters.SERVER_IMAGEPATH} -JCSVPath=${pipelineParameters.SERVER_CSVPATH} -JTargetHost=${SERVER_HOST_SUBSCRIBER} -j jmeter.save.saveservice.output_format=xml -n -t /home/centos/data/dam/staging-to-prod/${pipelineParameters.FINDSTATUS_JMX_FILE} -l /home/centos/test_logs/test_log_dam_staging_syncstatus_${nowDate.format(date)}.jtl\''
                                """
                        ...
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                ...
                                sh """
                                    ...
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && chmod 400 ./keys/Performance-Test-Keypair.pem'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && scp -r -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem jmeter/DAM_Staging_to_Production_Tests/* centos@performance-test-jmeter-master.team-q-dev.com:~/data/dam/staging-to-prod'
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && ssh -o StrictHostKeyChecking=no -i ./keys/Performance-Test-Keypair.pem centos@performance-test-jmeter-master.team-q-dev.com \'env JVM_ARGS="-Dnashorn.args=--no-deprecation-warning" jmeter -JProtocol=${pipelineParameters
                                    .SERVER_PROTOCOL} -JHost=${SERVER_HOST_SUBSCRIBER} -JPort=${pipelineParameters.SERVER_PORT} -JRingAPIPort='' -JDAMPort='' -JImagePath=${pipelineParameters.SERVER_IMAGEPATH} -JCSVPath=${pipelineParameters.SERVER_CSVPATH} -j jmeter.save.saveservice.output_format=xml -n -t /home/centos/data/dam/staging-to-prod/${pipelineParameters.VERIFY_JMX_FILE} -l /home/centos/test_logs/test_log_dam_staging_validate_asset_${nowDate.format(date)}.jtl\''
                                """
        ...
        stage('Undeploying the staging application in k8 environment') {
            steps {
                script {
                    buildParams = []
                    buildParams.add(
                        [$class: 'StringParameterValue',
                            name: 'INSTANCE_NAME',
                            value: "${pipelineParameters.NAMESPACE}-sync-publisher"])
         ...
         stage('Undeploying the production application in k8 environment') {
            steps {
                script {
                    buildParams = []
                     buildParams.add(
                        [$class: 'StringParameterValue',
                            name: 'INSTANCE_NAME',
                            value: "${pipelineParameters.NAMESPACE}-sync-subscriber"])
                    build(job: "${pipelineParameters.KUBE_UNDEPLOY_JOB}", 
                          parameters: buildParams, 
                          propagate: true,
                          wait: true)
