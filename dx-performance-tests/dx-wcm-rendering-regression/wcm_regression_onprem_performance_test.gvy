...
pipeline {
    ...
    stages {
        ...
        stage("Load modules and configuration") {
            steps {
                script {
                    dxParametersLoadFromFile(pipelineParameters, "${env.WORKSPACE}/dx-performance-tests/dx-wcm-rendering-regression/parameters.yaml")
                }
            }
        }
                ...
                dir("${workspace}/dx-performance-tests/dx-wcm-rendering-regression") {
                    script {
                        ...
                        // Defines the CF Version to be deployed
                            env.CF_VERSION = pipelineParameters.CF_VERSION
                            env.INSTANCE_IP = pipelineParameters.INSTANCE_IP
                            env.GTEST_DURATION = pipelineParameters.GTEST_DURATION
                            env.GVUSERS_PER_TYPE = pipelineParameters.GVUSERS_PER_TYPE
                            env.JMETER_INSTANCE_IP = pipelineParameters.JMETER_INSTANCE_IP
                            env.JMETER_AGENT1 = pipelineParameters.JMETER_AGENT1
                            env.JMETER_AGENT2 = pipelineParameters.JMETER_AGENT2
         ...
         */
        stage('Run Release update to a given DXBuildNumber') {
            steps {
                withCredentials([
                    ...
                    dir("${workspace}/dx-core-tests/scripts") {
                        sh """
                            scp -i ${connectKey} -o StrictHostKeyChecking=no -r portal-update-cf-onpremnjdc.sh root@${env.INSTANCE_IP}:/portal-update-cf-onpremnjdc.sh
                            sh run-portal-cf-update-onpremnjdc.sh root ${env.INSTANCE_IP} ${CF_VERSION} ${TF_VAR_BUILD_LABEL} ${connectKey}
                        """
                    }
                }
            }
