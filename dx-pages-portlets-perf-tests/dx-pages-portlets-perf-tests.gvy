    ...
    stages {
        stage('Load modules and configuration') {
            steps {
                script {
                    ...
                    dxParametersLoadFromFile(pipelineParameters, "${env.WORKSPACE}/dx-pages-portlets-perf-tests/parameters.yaml")
                }
            }
        }
        ...
        stage('Prepare Settings') {
            steps {
                 ...
                 */
                dir("${WORKSPACE}/dx-pages-portlets-perf-tests") {
                    script {
                        ...
                        terraformVarsEC2.tfvar_instance_name = 'pages_portlets_test'
                         // instance area = PERF
                        ...
                        if(config == "medium"){
                            xmlFileToImport = "pagesandPortletsToImport_medium.xml"
                        } else if(config == "large"){
                            xmlFileToImport = "pagesandPortletsToImport_large.xml"
                        } else {
                             xmlFileToImport = "pagesandPortletsToImport.xml"
                        }
                        echo "portlets xml to import: ${xmlFileToImport}"
        ...
        stage('Look up for available dedicated hosts') {
            ...
            steps {
                script {
                    try {
                        dedicatedHostParameters.tfvar_dedicated_host_owner = dxJenkinsGetJobOwner(defaultOwner: 'srinath.tr@hcl.com')
            ...
            steps {
                configFileProvider([
                    ...
                    sh """
                        ...
                        scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${workspace}/dx-pages-portlets-perf-tests/helpers/01-setup-prereqs.sh centos@${terraformVarsEC2.instance_private_ip}:/tmp
                        ...
                        ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'sudo mkdir -p /opt/pnp-performance-test-reports && sudo chown centos: /opt/pnp-performance-test-reports'
        ...
        stage('Deploying the application in k8 environment for regression test') {
            ...
            steps {
                script {
                    dir("${WORKSPACE}/dx-pages-portlets-perf-tests") {
                        deploymentSettings = readYaml file: "${workspace}/dx-pages-portlets-perf-tests/deployment-settings.yaml"
        ...
        /*
         * Install dxclient, configure and import portlets & pages and set permissions
         */
        stage('Install dxclient, configure and import portlets & pages and set permissions') {
            steps {
                configFileProvider([
                    configFile(fileId: 'test-automation-deployments', variable: 'DEPLOY_KEY')
                ])
                          ...
                          [$class: 'UsernamePasswordMultiBinding', credentialsId: "${pipelineParameters.TOOL_CREDENTIALS_ID}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']
                                ...
                                try {
                                    sh """
                                        ...
                                        scp -r -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${workspace}/dx-pages-portlets-perf-tests/helpers/${xmlFileToImport} centos@${terraformVarsEC2.instance_private_ip}:/tmp
                    ...
                    configFileProvider([
                        ...
                        script {
                            try {
                                ...
                                sh """
                                    scp -v -i test-automation-deployments.pem -o StrictHostKeyChecking=no ./helpers/WCMLib.zip centos@${pipelineParameters.PORTAL_HOST}:/home/centos/
                                    scp -v -i test-automation-deployments.pem -o StrictHostKeyChecking=no ./helpers/ScriptAppLibraryFinalTest.zip centos@${pipelineParameters.PORTAL_HOST}:/home/centos/
                                    scp -v -i test-automation-deployments.pem -o StrictHostKeyChecking=no ./helpers/importWCMScriptAppLibs.sh centos@${pipelineParameters.PORTAL_HOST}:/home/centos/
                                """
        ...
        */
        stage('Import Logstash script to fetch pod logs and start logstash') {
         ...
         /*
         * Run JMeter performance tests for Pages and Portlets rendering
         */
        stage('Run JMeter performance tests for Pages and Portlets rendering') {
            steps {
                configFileProvider([
                    configFile(fileId: 'test-automation-deployments', variable: 'DEPLOY_KEY')
                ]) {
                    script {
                        ...
                        def date = new Date()
                        jtlFileRendering  = "test_log_pnp_rendering_${nowDate.format(date)}.jtl"
                        Exception caughtException = null
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                echo 'Running tests pnp rendering tests'

                                sh """
                                    chmod 600 ${DEPLOY_KEY}
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'env JVM_ARGS="-Dnashorn.args=--no-deprecation-warning" /home/centos/${pipelineParameters.JMETER_BINARY_VERSION}/bin/./jmeter.sh -JProtocol=${pipelineParameters.SERVER_PROTOCOL} -JHost=${pipelineParameters.PORTAL_HOST} -JUSERS_CSV_PATH=${pipelineParameters.USERS_CSV_PATH} -j jmeter.save.saveservice.output_format=xml -n -t /opt/Portal-Performance-Tests/jmeter/PageAndPortlet/${pipelineParameters.TARGET_JMX_FILE_FOR_Rendering} -JVUSERS_PER_TYPE=${pipelineParameters.RENDER_VUSERS_PER_TYPE} -JTEST_AUTHENTICATED_VUSER_RAMP=${pipelineParameters.RENDER_TEST_AUTHENTICATED_VUSER_RAMP} -JTEST_DURATION=${pipelineParameters.RENDER_TEST_DURATION} -JTOTAL_PAGES=${pipelineParameters.TOTAL_PAGES}  -l /home/centos/test_logs/${jtlFileRendering} -e -f -o /opt/pnp-performance-test-reports/pnp_Rendering'
                                """
                                ...
                                sh """
                                    scp -v -i ${DEPLOY_KEY} -o StrictHostKeyChecking=no ${workspace}/test-scripts/get_jtl_errors.sh centos@${terraformVarsEC2.instance_private_ip}:/opt/pnp-performance-test-reports/pnp_Rendering
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'sh /opt/pnp-performance-test-reports/pnp_Rendering/get_jtl_errors.sh "/opt/pnp-performance-test-reports/pnp_Rendering/content/js/dashboard.js" "/opt/pnp-performance-test-reports/pnp_Rendering/statistics.json"'
                                    scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip}:/home/centos/jtl_errors.json ${workspace}/dx-pages-portlets-perf-tests
                                """
                                ...
                                if ("${jmeter_error}" > 0) {
                                    currentBuild.result = 'UNSTABLE'
                                    echo 'Errors in pnp rendering assets JMeter script execution results'
                                }

                                // fetch average response and throughput from jmeter report
                                sh """
                                    chmod 600 ${DEPLOY_KEY}
                                    scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${workspace}/dx-pages-portlets-perf-tests/helpers/get_response_time.sh centos@${terraformVarsEC2.instance_private_ip}:/opt/Portal-Performance-Tests
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Performance-Tests && sh get_response_time.sh "/opt/pnp-performance-test-reports/pnp_Rendering/content/js/dashboard.js"'
                                    scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip}:/home/centos/pnp_performance_results.log ${workspace}/dx-pages-portlets-perf-tests
                                    echo "${currentBuild.displayName}," >> ${workspace}/dx-pages-portlets-perf-tests/pnp_performance_results.log
                                    cat ${workspace}/dx-pages-portlets-perf-tests/pnp_performance_results.log
                                """
         ...
         /*
         * Extract pages and portlets performance results from log file and push results to OpenSearch Dashboard
         */
                ...
                withCredentials([
                    ...
                    configFile(fileId: 'test-automation-deployments', variable: 'DEPLOY_KEY')
                            ...
                            script {
                                ...
                                echo "Pages and Portlets Regression Build_Time: - $build_Time"
                                Exception caughtException = null
                                catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                                    try {
                                        echo 'Running tests'
                                        sh """
                                            chmod 600 ${DEPLOY_KEY}
                                            sh ${workspace}/test-scripts/portlets-results-to-opensearch.sh ${workspace} "${pipelineParameters.OS_PROTOCOL}" "${pipelineParameters.OS_HOSTNAME}" "${pipelineParameters.OS_INDEX_NAME}" "${pipelineParameters.OS_USER_NAME}" "${OPENSEARCH_PASSWORD}" ${build_Time}
                                        """
                                    } catch (org.jenkinsci.plugins.workflow.steps.FlowInterruptedException e) {
