...
java -jar /opt/dam-staging-resync-acceptance-test-reports/Saxon-HE-9.5.1-3.jar -o:"$current_run_file" Dam_Staging_Resync_Acceptance_Test_Master.xml /opt/dam-staging-resync-acceptance-test-reports/dam-staging-resync-acceptance-test-run-to-xml.xslt pBuildName="$1"

# insert a line break needed for the read line while loop below
echo '' >> "$current_run_file"

# if there is a running WTF results file
dam_staging_resync_acceptance_test_combined_runs_file="dam-staging-resync-acceptance-test-runs.xml"
if [ -f $dam_staging_resync_acceptance_test_combined_runs_file ]
then
    # test to see if an entry for this build already exists
    if grep -Fq "$1" $dam_staging_resync_acceptance_test_combined_runs_file
    then
        ...
        do
            ...
            sed -i "/<results>/a $line1" "$dam_staging_resync_acceptance_test_combined_runs_file"
        done < "$current_run_file"
    fi

    # produce an updated WTF dashboard results HTML file that includes this run's results
    java -jar /opt/dam-staging-resync-acceptance-test-reports/Saxon-HE-9.5.1-3.jar -o:"$2"-dashboard.html "$dam_staging_resync_acceptance_test_combined_runs_file" /opt/dam-staging-resync-acceptance-test-reports/dashboard-convert-xml.xslt pWTF="$2"
    ...
    cp "$dam_staging_resync_acceptance_test_combined_runs_file" ./dashboard/"$2"-runs.xml
