...
java -jar /opt/git/Saxon-HE-9.5.1-3.jar -o:"$current_run_file" Test_IUT_Master.xml /opt/git/report/wtf-run-to-xml.xslt pBuildName="$1"

# insert a line break needed for the read line while loop below
echo '' >> "$current_run_file"

# if there is a running WTF results file
iut_combined_runs_file="iut-combined-runs.xml"
...
then
    ...
    java -jar /opt/git/Saxon-HE-9.5.1-3.jar -o:"$2"-dashboard.html "$iut_combined_runs_file" /opt/git/report/dashboard-convert-xml.xslt pWTF="$2"

    # ensure that we have a dashboard dir
    if [ ! -d "/opt/git/dashboard" ] 
    then
        echo "Directory /opt/git/dashboard DOES NOT exist, so creating it" 
        cd /opt/git
        mkdir dashboard
        cd /opt/git/report
    fi
    ...
    cp "$2"-dashboard.html /opt/git/dashboard/
    cp "$iut_combined_runs_file" /opt/git/dashboard/"$2"-combined-runs.xml
    cp wtf.css /opt/git/dashboard/
fi
