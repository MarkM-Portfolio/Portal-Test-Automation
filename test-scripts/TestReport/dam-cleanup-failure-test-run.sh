...
current_run_file="Dam_Cleanup_Tests_Results.xml"

cd /opt/dam-cleanup-test-reports

# if there is a running WTF results file
dam_cleanup_acceptance_test_combined_runs_file="dam-cleanup-test-runs.xml"
if [ -f $dam_cleanup_acceptance_test_combined_runs_file ]
then
    # test to see if an entry for this build already exists
    if grep -Fq "$1" $dam_cleanup_acceptance_test_combined_runs_file
    then
        ...
        do
            ...
            sed -i "/<results>/a $line1" "$dam_cleanup_acceptance_test_combined_runs_file"
        done < "$current_run_file"
    fi

    # produce an updated WTF dashboard results HTML file that includes this run's results
    java -jar /opt/dam-cleanup-test-reports/Saxon-HE-9.5.1-3.jar -o:"$2"-dashboard.html "$dam_cleanup_acceptance_test_combined_runs_file" /opt/dam-cleanup-test-reports/dam-cleanup-dashboard-failure-report-convert-xml.xslt pWTF="$2"
    ...
    cp "$dam_cleanup_acceptance_test_combined_runs_file" ./dashboard/"$2"-runs.xml
