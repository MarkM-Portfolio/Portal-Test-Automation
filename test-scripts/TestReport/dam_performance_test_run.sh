...
current_run_file="Dam_Performance_Current_Tests.xml"

cd /opt/dam-performance-test-reports

if [ ! -f "Dam_Performance_Tests_Results.xml" ]
then
    exit 1
fi
...
cat Dam_Performance_Tests_Results.xml
# produce xml that has combined results for this jenkins run (ends up being a single line in the output xml file)
java -jar /opt/dam-performance-test-reports/Saxon-HE-9.5.1-3.jar -o:"$current_run_file" Dam_Performance_Tests_Results.xml /opt/dam-performance-test-reports/dam_performance_test_run_to_xml.xslt pBuildName="$1"

# insert a line break needed for the read line while loop below
echo '' >> "$current_run_file"

# if there is a running WTF results file
dam_performance_test_combined_runs_file="dam-performance-tests-combined-runs.xml"
...
then
    ...
    java -jar /opt/dam-performance-test-reports/Saxon-HE-9.5.1-3.jar -o:"$2"-dashboard.html "$dam_performance_test_combined_runs_file" /opt/dam-performance-test-reports/dam_performance_test_convert_dashboard.xslt
