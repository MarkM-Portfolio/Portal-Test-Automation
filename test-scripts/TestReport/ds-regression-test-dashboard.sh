...
current_run_file="ds-regression-run.xml"

cd /opt/ds-perfomance-regression-reports/
...
# java -jar /opt/ds-perfomance-regression-reports/Saxon-HE-9.5.1-3.jar -o:"$current_run_file" Test_IUT_Master.xml /opt/ds-perfomance-regression-reports/wtf-run-to-xml.xslt pBuildName="$1"

# insert a line break needed for the read line while loop below
echo '' >> "$current_run_file"

# if there is a running WTF results file
iut_combined_runs_file="ds-regression-test-combined-runs.xml"
...
then
    ...
    java -jar /opt/ds-perfomance-regression-reports/Saxon-HE-9.5.1-3.jar -o:"$2"-dashboard.html "$iut_combined_runs_file" /opt/ds-perfomance-regression-reports/ds-dashboard-convert-xml.xslt pWTF="$2"

    # ensure that we have a dashboard dir
    if [ ! -d "/opt/ds-perfomance-regression-reports/dashboard" ] 
    then
        echo "Directory /opt/ds-perfomance-regression-reports/dashboard DOES NOT exist, so creating it" 
        cd /opt/ds-perfomance-regression-reports/
        mkdir dashboard
        cd /opt/ds-perfomance-regression-reports
    fi
    ...
    cp "$2"-dashboard.html /opt/ds-perfomance-regression-reports/dashboard/
    cp "$iut_combined_runs_file" /opt/ds-perfomance-regression-reports/dashboard/"$2"-combined-runs.xml
    cp wtf.css /opt/ds-perfomance-regression-reports/dashboard/
fi
