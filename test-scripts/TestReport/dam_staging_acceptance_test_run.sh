...
echo "running dam_staging_acceptance_test_run.sh"
current_run_file="Dam_Staging_Tests_Results.xml"

cd /opt/dam-staging-acceptance-test-reports

# if there is a running WTF results file
dam_staging_test_combined_runs_file="dam-staging-acceptance-test-runs.xml"
echo "dam-staging-acceptance-test-runs.xml ${dam_staging_test_combined_runs_file}"

if [ -f $dam_staging_test_combined_runs_file ]
then

    echo "{$1}"
    # test to see if an entry for this build already exists
    if grep -Fq "$1" $dam_staging_test_combined_runs_file
    then
        ...
        do
            ...
            sed -i "/<results>/a $line1" "$dam_staging_test_combined_runs_file"
        done < "$current_run_file"
    fi

    # produce an updated WTF dashboard results HTML file that includes this run's results
    java -jar /opt/dam-staging-acceptance-test-reports/Saxon-HE-9.5.1-3.jar -o:"$2"-dashboard.html "$dam_staging_test_combined_runs_file" /opt/dam-staging-acceptance-test-reports/dam-staging-acceptance-test-convert-xml.xslt pWTF="$2"
    ...
    cp "$dam_staging_test_combined_runs_file" ./dashboard/"$2"-runs.xml
