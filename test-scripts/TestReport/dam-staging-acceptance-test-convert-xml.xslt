...
<body>
  <div>
    ...
    <h2 class="top-h2"> <xsl:value-of select="$pWTF"/> Test Results </h2>
  </div>
    <!-- Table filter -->
  ...
  <table id="resultsTable" border="5">
    ...
    <xsl:for-each select="results/dam_staging_test">
      <tr>
        <td>
          <a>
        ...
        <!-- <td class="center-text"><xsl:value-of select="@failures"/></td> -->
        <td>
          <xsl:attribute name="class">
            <xsl:choose>
                <xsl:when test="number(translate(@failures, ',', '')) &gt; 0">
          ...
          <xsl:value-of select="translate(@failures, ',', '')"/>
        </td>
      </tr>
    </xsl:for-each>
  </table>
