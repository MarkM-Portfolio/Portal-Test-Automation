...
java -jar /opt/acceptance-test-reports/Saxon-HE-9.5.1-3.jar -o:"$current_run_file" Acceptance_Test_Master.xml /opt/acceptance-test-reports/acceptance-test-run-to-xml.xslt pBuildName="$1"

# insert a line break needed for the read line while loop below
echo '' >> "$current_run_file"

# if there is a running WTF results file
acceptance_test_combined_runs_file="acceptance-test-combined-runs.xml"
if [ -f $acceptance_test_combined_runs_file ]
then
    # test to see if an entry for this build already exists
    if grep -Fq "$1" $acceptance_test_combined_runs_file
    then
        ...
        do
            ...
            sed -i "/<results>/a $line1" "$acceptance_test_combined_runs_file"
        done < "$current_run_file"
    fi

    # produce an updated WTF dashboard results HTML file that includes this run's results
    java -jar /opt/acceptance-test-reports/Saxon-HE-9.5.1-3.jar -o:"$2"-dashboard.html "$acceptance_test_combined_runs_file" /opt/acceptance-test-reports/dashboard-convert-xml.xslt pWTF="$2"
    ...
    cp "$acceptance_test_combined_runs_file" ./dashboard/"$2"-combined-runs.xml
