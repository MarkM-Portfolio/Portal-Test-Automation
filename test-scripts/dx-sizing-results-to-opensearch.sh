...
# DX Sizing rendering performance results from log file extracted and pushed to OpenSearch dashboard
...
temp_file_in="${workspace}/dx-sizing-perf-tests/combined_script_rendering_performance_results.log"
file_in="${workspace}/dx-sizing-perf-tests/combined_script_rendering_performance_results.log"
...
# jtl error detials
jtl_error_json=$(cat ${workspace}/dx-sizing-perf-tests/jtl_errors.json)
...
do
    case $i in
        ...
        1)
            ...
            ;;
        2)
            totalSamples=$(echo ${resultArray[$i]} | tr -d ' ')
            echo "totalSamples - $totalSamples"
            ;;
        3)
            errorPercentage=$(echo ${resultArray[$i]} | tr -d ' ')
            echo "errorPercentage - $errorPercentage"
            ;;
        4)
...
\"testTime\": \"${buildTime}\",
\"configurationType\": \"${dxConfiguration}\",
\"cpuUsage\":  $cpu_values,
...
echo "DX Sizing ${dxConfiguration} config rendering results posted to OpenSearch Dashboard successfully !!"
