                ...
                withCredentials([
                    ...
                    dir("${workspace}/dx-onpremise/terraform/ec2-dx-onpremise-launch") {
                        configFileProvider([
                                ...
                                """)
                            sh """
                                mkdir ${workspace}/dx-onpremise/ihs
                                curl https://artifactory.cwp.pnp-hcl.com/artifactory/quintana-generic/dx-build-prereqs/image_prereqs/was.repo.9000.ihs.zip -o ${workspace}/dx-onpremise/ihs/was.repo.9000.ihs.zip --create-dirs
                                curl https://artifactory.cwp.pnp-hcl.com/artifactory/quintana-generic/dx-build-prereqs/image_prereqs/was.repo.9000.plugins.zip -o ${workspace}/dx-onpremise/ihs/was.repo.9000.plugins.zip --create-dirs
                                curl https://artifactory.cwp.pnp-hcl.com/artifactory/quintana-generic/dx-build-prereqs/image_prereqs/was.repo.9000.wct.zip -o ${workspace}/dx-onpremise/ihs/was.repo.9000.wct.zip --create-dirs
                                ls ${workspace}/dx-onpremise/ihs
                            """
                            sh """
                            ...
                            """
                            sh """
                                scp -i test-automation-deployments.pem -o StrictHostKeyChecking=no -r ${workspace}/dx-onpremise/ihs/* centos@${env.INSTANCE_IP}:/tmp/dx-onpremise/ihs
                                ssh -i test-automation-deployments.pem -o StrictHostKeyChecking=no centos@${env.INSTANCE_IP} \
        ...
        stage('Configure IHS For Cluster') {
            ...
            steps {
                dir("${workspace}/dx-onpremise/terraform/ec2-dx-onpremise-launch") {
                    sh """
                        ssh -i test-automation-deployments.pem -o StrictHostKeyChecking=no centos@${env.INSTANCE_IP} \
                        '(sudo sh /opt/HCL/ihsToolbox/WCT/wctcmd.sh -tool pct -defLocPathname /opt/HCL/ihsPlugins -defLocName DefaultDefinition -createDefinition -response /tmp/dx-onpremise/scripts/helpers/http-server/pct_responsefile.txt)'
                    """

                    sh """
                        ssh -i test-automation-deployments.pem -o StrictHostKeyChecking=no centos@${env.INSTANCE_IP} \
                        '(cd /tmp/dx-onpremise/scripts && sudo sh cluster/ihs-setup.sh ip-${env.DMGR_HOSTNAME}Node ${params.INSTANCE_NAME}${env.DOMAIN_SUFFIX} ${env.DX_USERNAME} ${env.DX_PASSWORD})'
                    """
                }
            }
        }
        ...
        stage('Configure IHS for standalone') {
            ...
            steps {
                dir("${workspace}/dx-onpremise/terraform/ec2-dx-onpremise-launch") {
                    ...
                    sh """
                        ssh -i test-automation-deployments.pem -o StrictHostKeyChecking=no centos@${env.INSTANCE_IP} \
                        '(cd /tmp/dx-onpremise/scripts && sudo sh ihs-setup-for-standalone.sh ip-${env.DMGR_HOSTNAME}Node ${params.INSTANCE_NAME}${env.DOMAIN_SUFFIX} ${env.DX_USERNAME} ${env.DX_PASSWORD})'
                    """
                }
            }
        }
        ...
        stage('Enable SSL for IHS') {
            steps {
                dir("${workspace}/dx-onpremise/terraform/ec2-dx-onpremise-launch") {
                    ...
                    sh """
                        ssh -i test-automation-deployments.pem -o StrictHostKeyChecking=no centos@${env.INSTANCE_IP} \
                        '(sudo sh /tmp/dx-onpremise/scripts/enable-ssl-for-ihs.sh ${params.INSTANCE_NAME}${env.DOMAIN_SUFFIX} ip-${env.DMGR_HOSTNAME}Node ${env.DX_USERNAME} ${env.DX_PASSWORD} ${params.CLUSTERED_ENV})'
                    """
