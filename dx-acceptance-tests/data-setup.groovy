...
pipeline {
    ...
    stages {
            ...
            steps {
                ...
                dir("${WORKSPACE}/dx-acceptance-tests") {
                    script {
                        ...
                        dxParametersLoadFromFile(pipelineParameters, 'data-setup.yaml')
                        ...
                        pipelineParameters.hostname = "DX_Data_Setup_${dateFormat.format(date)}"
        ...
        stage('Pull tests') {
            when {
                expression { 
                    pipelineParameters.SETUP_CC == true 
                }
            }
            steps {
                configFileProvider([
                        ...
                        catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                            try {
                                ...
                                sh """
                                    ...
                                    ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/content-ui/packages/ui && make data-setup-endpoint dx_core=${pipelineParameters.PORTAL_HOST} ring_api=${pipelineParameters.EXP_API} insecure=${pipelineParameters.SSL_ENABLED}'
                                """
