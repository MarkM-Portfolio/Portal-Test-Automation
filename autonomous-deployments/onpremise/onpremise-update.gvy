...
pipeline {
    ...
    parameters {
      ...
      string(name: 'ONPREMISE_UPGRADE_JOB', defaultValue: 'hybrid/onpremise/dx-onpremise-upgrade', description: 'Job which upgrades the onpremise environment',  trim: false)
    }
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "dx-onpremise-update"
                }
            }
        }
        ...
        stage('Updating the application in onpremise environment') {
            steps {
                script {
                    buildParams = commonModule.createOnpremiseUpgradeParams(env.NAMESPACE, commonConfig.COMMON_CLUSTERED_ENV, commonConfig.COMMON_ADDITIONAL_NODE)
                    imagesPushResult = build(job: "${params.ONPREMISE_UPGRADE_JOB}", 
                          parameters: buildParams, 
                          propagate: true,
                          wait: true)
                }
