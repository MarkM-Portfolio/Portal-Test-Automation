...
pipeline {
    ...
    parameters {
      string(name: 'DEPLOYMENT_LEVEL', defaultValue: 'master', description: 'Deploying master images')
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.INSTANCE_NAME = "native-kube-mstr-${DEPLOYMENT_METHOD}-fresh"
                    if (!env.MS_TEAMS_URL){
        ...
        stage('Run acceptance test for native kube master deployment') {
            steps {
                script {
