...
pipeline {
    ...
    parameters {
      string(name: 'DEPLOYMENT_LEVEL', defaultValue: 'release', description: 'Deploying release images')
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.INSTANCE_NAME = "native-kube-rel-${DEPLOYMENT_METHOD}-fresh"
                    if (!env.MS_TEAMS_URL){
        ...
        stage('Deploying the application in k8 environment') {
            steps {
                script {
                    buildParams = commonModule.createKubeParams(env.INSTANCE_NAME, params.KUBE_FLAVOUR, "", "", "", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH, params.DEPLOYMENT_LEVEL, params.DEPLOYMENT_METHOD, params.DOMAIN_SUFFIX)
        ...
        stage('Run acceptance test for native kube release deployment') {
            steps {
                script {
                    buildParams = commonModule.createAcceptanceTestJobParams(env.INSTANCE_NAME, params.KUBE_FLAVOUR, params.DEPLOYMENT_LEVEL, DEPLOYMENT_TYPE_KUBE, "", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH)
