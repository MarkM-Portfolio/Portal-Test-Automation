...
pipeline {
    ...
    stages {
        ...
        stage('Run acceptance test for AWS EKS fresh deployment') {
            steps {
                script {
    ...
    post {
        ...
        success {
            script {
                office365ConnectorSend message: "Build Success ${env.JOB_NAME} commited by @${user} [View on Jenkins] https://${env.NAMESPACE}.hcl-dx-dev.net/${env.CONTEXT_ROOT_PATH}/${env.DX_CORE_HOME_PATH} ", status: "Build Success", webhookUrl: "${env.MS_TEAMS_URL}"
            }
        }
