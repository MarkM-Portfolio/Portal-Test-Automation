...
pipeline {
    ...
    parameters {
      ...
      string(name: 'KUBE_UNDEPLOY_JOB', defaultValue: 'kube/cloud-undeploy', description: 'Job which undeploys the image in gke',  trim: false)
      string(name: 'KUBE_DEPLOY_JOB', defaultValue: 'kube/cloud-deploy', description: 'Job which deploys the image in gke',  trim: false)
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "gke-dev-${DEPLOYMENT_METHOD}-fresh"
                }
            }
        }
        ...
        stage('Run acceptance test for Google GKE develop deployment') {
            steps {
                script {
