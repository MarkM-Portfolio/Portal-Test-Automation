...
pipeline {
    ...
    parameters {
      ...
      string(name: 'CLUSTER_NAME', defaultValue: 'dx-perf-cluster', description: 'Cluster name',  trim: false)
      string(name: 'CLUSTER_REGION', defaultValue: 'us-east4-a', description: 'Cluster region',  trim: false)
      ...
      string(name: 'DAM_RTRM_PERFORMANCE', defaultValue: 'true', description: 'To Enable DAM RTRM Performance')
    ...
    stages {
        stage("Load modules and configuration for RTRM") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "rtrm-${DEPLOYMENT_METHOD}-n${UPDATE_RELEASE_LEVEL}-dev"
                }
            }
        }
        ...
        stage('Undeploying the application in k8 environment for RTRM') {
            steps {
                script {
                    buildParams = []
                    buildParams = commonModule.createKubeUnDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.CLUSTER_NAME, CLUSTER_REGION, "")
        ...
        stage('Deploying the application in k8 environment for RTRM') {
            steps {
                script {
                    buildParams = commonModule.createKubeDeployParamsForMaster(env.NAMESPACE, params.KUBE_FLAVOUR, params.CLUSTER_NAME, CLUSTER_REGION, "", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH, UPDATE_RELEASE_LEVEL, params.DEPLOYMENT_LEVEL, params.DEPLOYMENT_METHOD, params.DOMAIN_SUFFIX)
        ...
        stage('Updating the application in k8 environment for RTRM') {
            steps {
                script {
                    buildParams = commonModule.createKubeParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.CLUSTER_NAME, CLUSTER_REGION, "", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH, params.UPDATE_DEPLOYMENT_LEVEL, params.DEPLOYMENT_METHOD, params.DOMAIN_SUFFIX)
