...
pipeline {
    ...
    parameters {
      ...
      string(name: 'UPDATE_DEPLOYMENT_LEVEL', defaultValue: 'release',description: 'Updating to the latest develop images')
      string(name: 'KUBE_FLAVOUR', defaultValue: 'google',description: 'Deploying the google gke hybrid environment.')
      string(name: 'HYBRID_UNDEPLOY_JOB', defaultValue: 'hybrid/hybrid-destroy', description: 'Job which undeploys the google gke hybrid environment',  trim: false)
      string(name: 'HYBRID_DEPLOY_JOB', defaultValue: 'hybrid/hybrid-deploy', description: 'Job which deploys the google gke hybrid environment',  trim: false)
      string(name: 'HYBRID_UPDATE_JOB', defaultValue: 'hybrid/hybrid-update', description: 'Job which updates the google gke hybrid environment',  trim: false)
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "gke-n${UPDATE_RELEASE_LEVEL}-clstrhyb-rel"
                }
            }
        }

        stage('Undeploying the application from google gke hybrid environment') {
            steps {
                script {
                    buildParams = []
                    if (!env.MS_TEAMS_URL){
        ...
        stage('Deploying the application in google gke hybrid environment') {
            steps {
                script {
        ...
        stage('Updating the application in google gke hybrid environment') {
            steps {
                script {
        ...
        stage('Run acceptance test on google gke hybrid environment') {
            steps {
                script {
