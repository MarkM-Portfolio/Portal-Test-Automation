...
pipeline {
    ...
    stages {
        ...
        stage('Deploying the application in openshift hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.DEPLOYMENT_LEVEL, '', UPDATE_RELEASE_LEVEL, commonConfig.COMMON_CLUSTERED_ENV, commonConfig.COMMON_ADDITIONAL_NODE, UPDATE_CORE_RELEASE_LEVEL)
        ...
        stage('Updating the application in openshift hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeUpdateParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.UPDATE_DEPLOYMENT_LEVEL, ".apps${DOMAIN_SUFFIX}", UPDATE_RELEASE_LEVEL, commonConfig.COMMON_CLUSTERED_ENV, commonConfig.COMMON_ADDITIONAL_NODE, UPDATE_CORE_RELEASE_LEVEL)
