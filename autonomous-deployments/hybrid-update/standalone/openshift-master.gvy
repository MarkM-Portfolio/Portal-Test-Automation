...
pipeline {
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "os-n${UPDATE_RELEASE_LEVEL}-stdhyb-mster"
                }
            }
        }
        ...
        stage('Deploying the application in openshift hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.DEPLOYMENT_LEVEL, '', UPDATE_RELEASE_LEVEL, "false", "false", UPDATE_CORE_RELEASE_LEVEL)
        ...
        stage('Updating the application in openshift hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeUpdateParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.UPDATE_DEPLOYMENT_LEVEL, ".apps${DOMAIN_SUFFIX}", UPDATE_RELEASE_LEVEL, "false", "false", UPDATE_CORE_RELEASE_LEVEL)
