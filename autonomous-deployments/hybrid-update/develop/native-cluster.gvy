...
pipeline {
    ...
    parameters {
      ...
      string(name: 'HYBRID_UPDATE_JOB', defaultValue: 'hybrid/hybrid-native-update', description: 'Job which updates the native kube hybrid environment',  trim: false)
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "native-n${UPDATE_RELEASE_LEVEL}-clstrhyb-dev"
                    if (!env.MS_TEAMS_URL){
        ...
        stage('Undeploying the native kube hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeUnDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, commonConfig.COMMON_CLUSTERED_ENV, "", "", "")
        ...
        stage('Deploying the native kube hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.DEPLOYMENT_LEVEL, params.DOMAIN_SUFFIX, params.UPDATE_RELEASE_LEVEL, commonConfig.COMMON_CLUSTERED_ENV, commonConfig.COMMON_ADDITIONAL_NODE, UPDATE_CORE_RELEASE_LEVEL)
        ...
        stage('Updating the native kube hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeUpdateParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.UPDATE_DEPLOYMENT_LEVEL, params.DOMAIN_SUFFIX, params.UPDATE_RELEASE_LEVEL, commonConfig.COMMON_CLUSTERED_ENV, commonConfig.COMMON_ADDITIONAL_NODE, UPDATE_CORE_RELEASE_LEVEL)
        ...
        stage('Run acceptance test on native kube hybrid environment') {
            steps {
                script {
    ...
    post {
        ...
        unstable {
            script {
                office365ConnectorSend message: "Build Success ${env.JOB_NAME} commited by @${user} [View on Jenkins] https://${env.NAMESPACE}${DOMAIN_SUFFIX}/wps/myportal", status: "Build Success. Acceptance test result is quite unstable", webhookUrl: "${env.MS_TEAMS_URL}"
            }
        }

        success {
            script {
                office365ConnectorSend message: "Build Success ${env.JOB_NAME} commited by @${user} [View on Jenkins] https://${env.NAMESPACE}${DOMAIN_SUFFIX}/wps/myportal", status: "Build Success", webhookUrl: "${env.MS_TEAMS_URL}"
            }
        }
