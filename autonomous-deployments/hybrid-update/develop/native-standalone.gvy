...
pipeline {
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "native-n${UPDATE_RELEASE_LEVEL}-stdhyb-dev"
                    if (!env.MS_TEAMS_URL){
        ...
        stage('Deploying the native kube hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.DEPLOYMENT_LEVEL, params.DOMAIN_SUFFIX, params.UPDATE_RELEASE_LEVEL, "false", "false", UPDATE_CORE_RELEASE_LEVEL)
        ...
        stage('Updating the native kube hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeUpdateParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.UPDATE_DEPLOYMENT_LEVEL, params.DOMAIN_SUFFIX, params.UPDATE_RELEASE_LEVEL, "false", "false", UPDATE_CORE_RELEASE_LEVEL)
