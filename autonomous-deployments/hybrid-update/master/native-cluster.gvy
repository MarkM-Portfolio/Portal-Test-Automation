...
pipeline {
        ...
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "native-n${UPDATE_RELEASE_LEVEL}-clstrhyb-mstr"
                    if (!env.MS_TEAMS_URL){
    ...
    post {
        ...
        unstable {
            script {
                office365ConnectorSend message: "Build Success ${env.JOB_NAME} commited by @${user} [View on Jenkins] https://${env.NAMESPACE}${DOMAIN_SUFFIX}/mydx", status: "Build Success. Acceptance test result is quite unstable", webhookUrl: "${env.MS_TEAMS_URL}"
            }
        }

        success {
            script {
                office365ConnectorSend message: "Build Success ${env.JOB_NAME} commited by @${user} [View on Jenkins] https://${env.NAMESPACE}${DOMAIN_SUFFIX}/mydx", status: "Build Success", webhookUrl: "${env.MS_TEAMS_URL}"
            }
        }
