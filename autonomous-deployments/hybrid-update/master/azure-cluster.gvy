...
pipeline {
    ...
    parameters {
      ...
      string(name: 'KUBE_FLAVOUR', defaultValue: 'azure',description: 'Deploying the azure aks hybrid environment.')
      string(name: 'HYBRID_UNDEPLOY_JOB', defaultValue: 'hybrid/hybrid-destroy', description: 'Job which undeploys the azure aks hybrid environment',  trim: false)
      string(name: 'HYBRID_DEPLOY_JOB', defaultValue: 'hybrid/hybrid-deploy', description: 'Job which deploys the azure aks hybrid environment',  trim: false)
      string(name: 'HYBRID_UPDATE_JOB', defaultValue: 'hybrid/hybrid-update', description: 'Job which updates the azure aks hybrid environment',  trim: false)
      ...
      string(name: 'RESOURCE_GROP', defaultValue: 'AZ-QUINTANA-RG-US-EAST', description: 'Resource Group of the cluster')
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "aks-n${UPDATE_RELEASE_LEVEL}-clstrhyb-mstr"
                }
            }
        }

        stage('Undeploying the application from azure aks hybrid environment') {
            steps {
                script {
        ...
        stage('Deploying the application in azure aks hybrid environment') {
            steps {
                script {
        ...
        stage('Updating the application in azure aks hybrid environment') {
            steps {
                script {
        ...
        stage('Run acceptance test on azure aks hybrid environment') {
            steps {
                script {
