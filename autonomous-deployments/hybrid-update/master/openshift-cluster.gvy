...
pipeline {
    ...
    parameters {
      ...
      string(name: 'UPDATE_DEPLOYMENT_LEVEL', defaultValue: 'master',description: 'Updating to the latest develop images')
      string(name: 'KUBE_FLAVOUR', defaultValue: 'openshift',description: 'Deploying the openshift os hybrid environment.')
      string(name: 'HYBRID_UNDEPLOY_JOB', defaultValue: 'hybrid/hybrid-destroy', description: 'Job which undeploys the openshift os hybrid environment',  trim: false)
      string(name: 'HYBRID_DEPLOY_JOB', defaultValue: 'hybrid/hybrid-deploy', description: 'Job which deploys the openshift os hybrid environment',  trim: false)
      string(name: 'HYBRID_UPDATE_JOB', defaultValue: 'hybrid/hybrid-update', description: 'Job which updates the openshift os hybrid environment',  trim: false)
      ...
      string(name: 'AWS_ZONE_ID', defaultValue: 'Z015304021BTM5FF7UP77', description: 'Zone Id of the Route 53 record')
      string(name: 'DOMAIN_SUFFIX', defaultValue: '.apps.dx-cluster-dev.hcl-dx-dev.net', description: 'Domain Suffix of the Route 53 record')
      string(name: 'KUBE_DOMAIN_SUFFIX', defaultValue: '.apps.dx-cluster-dev.hcl-dx-dev.net', description: 'Kube deployment domain suffix')
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "os-n${UPDATE_RELEASE_LEVEL}-clstrhyb-mstr"
                }
            }
        }

        stage('Undeploying the application from openshift os hybrid environment') {
            steps {
                script {
                    buildParams = []
                    if (!env.MS_TEAMS_URL){
                    ...
                    buildParams = commonModule.createHybridKubeUnDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, commonConfig.COMMON_CLUSTERED_ENV, params.CLUSTER_NAME, params.CLUSTER_REGION, params.RESOURCE_GROP)
        ...
        stage('Deploying the application in openshift os hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.DEPLOYMENT_LEVEL, DOMAIN_SUFFIX, UPDATE_RELEASE_LEVEL, commonConfig.COMMON_CLUSTERED_ENV, commonConfig.COMMON_ADDITIONAL_NODE, UPDATE_CORE_RELEASE_LEVEL)
        ...
        stage('Updating the application in openshift os hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridKubeUpdateParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.UPDATE_DEPLOYMENT_LEVEL, DOMAIN_SUFFIX, UPDATE_RELEASE_LEVEL, commonConfig.COMMON_CLUSTERED_ENV, commonConfig.COMMON_ADDITIONAL_NODE, UPDATE_CORE_RELEASE_LEVEL)
                    buildParams.add(
        ...
        stage('Run acceptance test on openshift os hybrid environment') {
            steps {
                script {
    ...
    post {
        ...
        unstable {
            script {
                office365ConnectorSend message: "Build Success ${env.JOB_NAME} commited by @${user} [View on Jenkins] https://${env.NAMESPACE}${DOMAIN_SUFFIX}/hcl/dx ", status: "Build Success. Acceptance test result is quite unstable", webhookUrl: "${env.MS_TEAMS_URL}"
            }
        }

        success {
            script {
                office365ConnectorSend message: "Build Success ${env.JOB_NAME} commited by @${user} [View on Jenkins] https://${env.NAMESPACE}${DOMAIN_SUFFIX}/hcl/dx", status: "Build Success", webhookUrl: "${env.MS_TEAMS_URL}"
            }
        }
