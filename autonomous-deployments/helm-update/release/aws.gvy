        ...
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    // Get UPDATE_RELEASE_LEVEL from JOB_BASE_NAME
                    // Then update from n to n+UPDATE_RELEASE_LEVEL
                    if (env.JOB_BASE_NAME.contains("-update-n-")) {
                        env.UPDATE_RELEASE_LEVEL = env.JOB_BASE_NAME.split("-update-n-")[1].split("-")[0]
                    } else {
                        env.UPDATE_RELEASE_LEVEL = "0"
                    }
