...
pipeline {
    ...
    parameters {
      ...
      string(name: 'KUBE_UNDEPLOY_JOB', defaultValue: 'kube/cloud-undeploy', description: 'Job which undeploys the image on EKS',  trim: false)
      string(name: 'KUBE_DEPLOY_JOB', defaultValue: 'kube/cloud-deploy', description: 'Job which deploys the image on EKS',  trim: false)
      string(name: 'KUBE_UPDATE_JOB', defaultValue: 'kube/cloud-update-deploy', description: 'Job which updates the image on EKS',  trim: false)
        ...
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "eks-${DEPLOYMENT_METHOD}-n${UPDATE_RELEASE_LEVEL}-rel"
                }
            }
        }
