...
pipeline {
    ...
    parameters {
      ...
      string(name: 'KUBE_FLAVOUR', defaultValue: 'azure',description: 'Deploying to the aws environment.')
      string(name: 'KUBE_UNDEPLOY_JOB', defaultValue: 'kube/cloud-undeploy', description: 'Job which undeploys the image in aks',  trim: false)
      string(name: 'KUBE_DEPLOY_JOB', defaultValue: 'kube/cloud-deploy', description: 'Job which deploys the image in aks',  trim: false)
        ...
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "aks-${DEPLOYMENT_METHOD}-n${UPDATE_RELEASE_LEVEL}-dev"
                }
            }
        }
