...
pipeline {
    ...
    parameters {
      ...
      string(name: 'UPDATE_DEPLOYMENT_LEVEL', defaultValue: 'master',description: 'Deploying rivendell master images')
      string(name: 'KUBE_FLAVOUR', defaultValue: 'openshift',description: 'Deploying to the GKE environment.')
      string(name: 'KUBE_UNDEPLOY_JOB', defaultValue: 'kube/cloud-undeploy', description: 'Job which undeploys the image on openshift',  trim: false)
      string(name: 'KUBE_DEPLOY_JOB', defaultValue: 'kube/cloud-deploy', description: 'Job which deploys the image on openshift',  trim: false)
      string(name: 'KUBE_UPDATE_JOB', defaultValue: 'kube/cloud-update-deploy', description: 'Job which updates the image on openshift',  trim: false)
      string(name: 'UPDATE_RELEASE_LEVEL', defaultValue: '2', description: 'Update from n to n+UPDATE_RELEASE_LEVEL',  trim: false)
    ...
    stages {
        ...
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "os-${DEPLOYMENT_METHOD}-n${UPDATE_RELEASE_LEVEL}-mastr"
                }
            }
        }
        ...
        stage('Deploying the application in k8 environment') {
            steps {
                script {
                    buildParams = commonModule.createKubeDeployParamsForMaster(env.NAMESPACE, params.KUBE_FLAVOUR, "", "", "", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH, params.UPDATE_RELEASE_LEVEL, params.DEPLOYMENT_LEVEL, params.DEPLOYMENT_METHOD, params.DOMAIN_SUFFIX)
        ...
        stage('Updating the application in k8 environment') {
            steps {
                script {
                    sleep 1000
                    buildParams = commonModule.createKubeParams(env.NAMESPACE, params.KUBE_FLAVOUR, "", "", "", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH, params.UPDATE_DEPLOYMENT_LEVEL, params.DEPLOYMENT_METHOD, params.DOMAIN_SUFFIX)
