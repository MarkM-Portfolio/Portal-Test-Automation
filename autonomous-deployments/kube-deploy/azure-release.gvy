...
pipeline {
    ...
    parameters {
      ...
      string(name: 'KUBE_FLAVOUR', defaultValue: 'azure',description: 'Deploying to the AKS environment.')
      string(name: 'KUBE_UNDEPLOY_JOB', defaultValue: 'kube/cloud-undeploy', description: 'Job which undeploys the image on AKS',  trim: false)
      string(name: 'KUBE_DEPLOY_JOB', defaultValue: 'kube/cloud-deploy', description: 'Job which deploys the image on AKS',  trim: false)
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "aks-rel-${DEPLOYMENT_METHOD}-fresh"
                }
            }
        }
        ...
        stage('Undeploying the application in k8 environment') {
            steps {
                script {
                    ...
                    buildParams = commonModule.createKubeUnDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, "dx-cluster-auto1", "", "AZ-QUINTANA-RG-US-EAST")
        ...
        stage('Deploying the application in k8 environment') {
            steps {
                script {
                    buildParams = commonModule.createKubeParams(env.NAMESPACE, params.KUBE_FLAVOUR, "dx-cluster-auto1", "", "AZ-QUINTANA-RG-US-EAST", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH, params.DEPLOYMENT_LEVEL, params.DEPLOYMENT_METHOD, params.DOMAIN_SUFFIX)
        ...
        stage('Run acceptance test for Azure AKS release deployment') {
            steps {
                script {
