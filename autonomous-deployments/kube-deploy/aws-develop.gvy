...
pipeline {
    ...
    parameters {
      ...
      string(name: 'KUBE_FLAVOUR', defaultValue: 'aws',description: 'Deploying to the aws environment.')
      string(name: 'KUBE_UNDEPLOY_JOB', defaultValue: 'kube/cloud-undeploy', description: 'Job which undeploys the image in aws',  trim: false)
        ...
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "eks-dev-${DEPLOYMENT_METHOD}-fresh"
                }
            }
        }
