...
pipeline {
    ...
    parameters {
      ...
      string(name: 'KUBE_UNDEPLOY_JOB', defaultValue: 'kube/cloud-undeploy', description: 'Job which undeploys the image in azure',  trim: false)
      string(name: 'KUBE_DEPLOY_JOB', defaultValue: 'kube/cloud-deploy', description: 'Job which deploys the image in azure',  trim: false)
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "aks-dev-${DEPLOYMENT_METHOD}-fresh"
                }
            }
        }
        ...
        stage('Run acceptance test for Azure AKS development deployment') {
            steps {
                script {
