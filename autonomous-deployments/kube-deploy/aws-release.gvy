...
pipeline {
    ...
    parameters {
      ...
      string(name: 'KUBE_FLAVOUR', defaultValue: 'aws',description: 'Deploying to the EKS environment.')
      string(name: 'KUBE_UNDEPLOY_JOB', defaultValue: 'kube/cloud-undeploy', description: 'Job which undeploys the image on aws',  trim: false)
      string(name: 'KUBE_DEPLOY_JOB', defaultValue: 'kube/cloud-deploy', description: 'Job which deploys the image on aws',  trim: false)
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "eks-rel-${DEPLOYMENT_METHOD}-fresh"
                }
            }
        }
        ...
        stage('Undeploying the application in k8 environment') {
            steps {
                script {
                    ...
                    buildParams = commonModule.createKubeUnDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, "eks_cluster_dx02", "us-east-1", "")
        ...
        stage('Deploying the application in k8 environment') {
            steps {
                script {
                    buildParams = commonModule.createKubeParams(env.NAMESPACE, params.KUBE_FLAVOUR, "eks_cluster_dx02", "us-east-1", "", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH, params.DEPLOYMENT_LEVEL, params.DEPLOYMENT_METHOD, params.DOMAIN_SUFFIX)
        ...
        stage('Run acceptance test for AWS EKS release deployment') {
            steps {
                script {
