...
pipeline {
    ...
    stages {
        stage("Load modules and configuration") {
            steps {
                script {
                    ...
                    env.NAMESPACE = "os-stdhyb-${DEPLOYMENT_METHOD}-rel"
                }
            }
        }
        ...
        stage('Deploying the application in openshift hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridFreshDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.DEPLOYMENT_LEVEL, DOMAIN_SUFFIX, 0, "false", "false", 0, params.CLUSTER_NAME, params.CLUSTER_REGION, params.RESOURCE_GROP, params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH)
