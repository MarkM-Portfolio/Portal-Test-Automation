...
pipeline {
    ...
    stages {
        ...
        stage('Undeploying the application from openshift hybrid environment') {
            steps {
                script {
                    ...
                    imagesPushResult = build(job: "${params.HYBRID_UNDEPLOY_JOB}", 
        ...
        stage('Deploying the application in openshift hybrid environment') {
            steps {
                script {
                    buildParams = commonModule.createHybridFreshDeployParams(env.NAMESPACE, params.KUBE_FLAVOUR, params.DEPLOYMENT_LEVEL, DOMAIN_SUFFIX, 0, commonConfig.COMMON_CLUSTERED_ENV, commonConfig.COMMON_ADDITIONAL_NODE, 0, "", "", "", params.CONTEXT_ROOT_PATH, params.DX_CORE_HOME_PATH, params.PERSONALIZED_DX_CORE_PATH)
                    imagesPushResult = build(job: "${params.HYBRID_DEPLOY_JOB}", 
                          parameters: buildParams, 
                          propagate: true,
                          wait: true)
                }
