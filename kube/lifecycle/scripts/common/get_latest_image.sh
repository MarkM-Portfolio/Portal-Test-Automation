...
if [[ "$REGEX_PATTERN" != "" ]]; then
    regex=$REGEX_PATTERN
    # if filter includes the excludePJD pattern allow the PJD image tag
    if [[ "$REGEX_PATTERN" == *"$excludePJD"* ]]; then
        excludePJD="_-_-_-_"
    fi
fi
...
if [[ $artifactoryUrl == *".gcr."* ]]; then
    linearray=($(gcloud container images list-tags $artifactoryUrl/$image | grep $regex | grep -v "_debug" | grep -v "$excludePJD" |tr -s '[:blank:]' ',' | cut -d ',' -f 2 | sed 's/.*">//' | sed 's/\/.*>//'))
elif [[ $artifactoryUrl == *".ecr."* ]]; then
    registryId=$(echo $artifactoryUrl | cut -d'.' -f1)
    region=$(echo $artifactoryUrl | cut -d'.' -f4)
    linearray=($(aws ecr list-images --registry-id $registryId --repository-name $image --region $region --output text | grep $regex | grep -v "_debug" | grep -v "$excludePJD" |tr -s '[:blank:]' ',' | cut -d ',' -f 3 | sed 's/.*">//' | sed 's/\/.*>//'))
elif [[ $artifactoryUrl == *".azurecr."* ]]; then
    acrName=$(echo $artifactoryUrl | cut -d'.' -f1)
    linearray=($(az acr repository show-tags --name  $acrName --repository $image --output table | grep $regex | grep -v "_debug" | grep -v "$excludePJD" |tr -s '[:blank:]' ',' | cut -d ',' -f 3 | sed 's/.*">//' | sed 's/\/.*>//'))
else
    linearray=($(curl --silent "$artifactoryUrl/$image/" | grep $regex | grep -v "_debug" | grep -v "$excludePJD" |tr -s '[:blank:]' ',' | cut -d ',' -f 2 | sed 's/.*">//' | sed 's/\/.*>//' | sed 's/<//'))
fi
