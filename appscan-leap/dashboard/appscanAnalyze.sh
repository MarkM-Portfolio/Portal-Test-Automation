...
#
# Output at: https://dxcore-test-results-secure.team-q-dev.com/DX-95-TestReports/s3mount/appscan-source-leap/
#
# *****************************************************************
...
quiet="false"
verbose="false"
scanResultUrl="https://artifactory.cwp.pnp-hcl.com/artifactory/quintana-generic/appscan/leap-reports"
scanResultFolder=""
scanBaselineStamp=""
scanBaselineCompare=""
action=""
...
compareFindings () {
   ...
   getScanInfo "$CF_xmlline" "$infokey"
   ...
   for out in $AXB_intput; do
      ...
      if [ "$out" == "{{link}}" ]; then
         if [ "${AXB_outval[0]}" == "Baseline" ]; then
            ...
            if [[ "${AXB_outval[0]}" == "Develop"* ]] || [[ "${AXB_outval[0]}" == "Release"* ]]; then
...
singleAnalyze () {
   ...
   if [ "$action" == "--info" ]; then
       ...
       compareFindings "$ozasmtXML" "$applicationName"
   else
       echo "Nothing to do"
   fi
}
