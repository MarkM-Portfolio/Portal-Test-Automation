  ...
  // Runs in build_infra, since we are creating infrastructure
   ...
   stages {
      // Load the pipeline parameters, common modules and configuration
        ...
        stage('Validate PZN Data through newman testing') {
                options {
                        ...
                        script {
                            ...
                            catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                                try {
                                    ...
                                    copyArtifacts(projectName: 'newman_upload', filter: 'exported_PZN_environment.json')
                                    sh """
                                        chmod 600 ${DEPLOY_KEY}
                                        scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${env.WORKSPACE}/exported_PZN_environment.json centos@${terraformVarsEC2.instance_private_ip}:/opt/Portal-Test-Automation/newman/
                                    """
                                    sh """
                                        chmod 600 ${DEPLOY_KEY}
                                        ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Test-Automation/newman && sed -i "s|http://localhost:10039|https://${pipelineParameters.INSTANCE_NAME}${pipelineParameters.DOMAIN_SUFFIX}|g" /opt/Portal-Test-Automation/newman/localhost_API.postman_environment.json && npx newman run PZN_validate.postman_collection.json -e exported_PZN_environment.json'
                                    """
        ...
        stage('Validate CC/WCM Data through newman testing') {
                options {
                    timeout(time: 30, unit: 'MINUTES') 
                }
                steps {
                        ...
                        script {
                            ...
                            catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                                try {
                                    ...
                                    echo "EXIM API will be: ${env.NEW_EXP_API}"
                                    echo "INSTANCE_IP will be: ${terraformVarsEC2.instance_private_ip}"
                                    copyArtifacts(projectName: 'newman_upload', filter: 'exported_WCM_environment.json')
                                    sh """
                                        chmod 600 ${DEPLOY_KEY}
                                        scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${env.WORKSPACE}/exported_WCM_environment.json centos@${terraformVarsEC2.instance_private_ip}:/opt/Portal-Test-Automation/newman/
                                    """
                                    sh """
                                        chmod 600 ${DEPLOY_KEY}
                                        ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Test-Automation/newman && sed -i "s|http://localhost:10039|https://${pipelineParameters.INSTANCE_NAME}${pipelineParameters.DOMAIN_SUFFIX}|g" /opt/Portal-Test-Automation/newman/localhost_API.postman_environment.json && npx newman run wcmrest_categories_validate.postman_collection.json -e exported_WCM_environment.json '
                                    """
                                } catch (org.jenkinsci.plugins.workflow.steps.FlowInterruptedException e) {
                                    error "TIMEOUT ${e.toString()}"
                                } catch (Throwable e) {
        ...
        stage('Validate DAM Data through newman testing') {
                options {
                    timeout(time: 30, unit: 'MINUTES') 
                }
                steps {
                        ...
                        script {
                            ...
                            catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                                try {
                                    ...
                                    echo "INSTANCE_IP will be: ${terraformVarsEC2.instance_private_ip}"
                                    // Copy the artifact from the newman_upload stage to the workspace
                                    copyArtifacts(projectName: 'newman_upload', filter: 'exported_environment.json')
                                    // Copy the artifact from the workspace to the target machine
                                    sh """
                                        chmod 600 ${DEPLOY_KEY}
                                        scp -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} ${env.WORKSPACE}/exported_environment.json centos@${terraformVarsEC2.instance_private_ip}:/opt/Portal-Test-Automation/newman/
                                    """
                                    sh """
                                        chmod 600 ${DEPLOY_KEY}
                                        ssh -o StrictHostKeyChecking=no -i ${DEPLOY_KEY} centos@${terraformVarsEC2.instance_private_ip} 'cd /opt/Portal-Test-Automation/newman && sed -i "s|http://localhost:10039|https://${pipelineParameters.INSTANCE_NAME}${pipelineParameters.DOMAIN_SUFFIX}|g" /opt/Portal-Test-Automation/newman/localhost_DAM.postman_environment.json && npx newman run DAM_Validate.postman_collection.json -e exported_environment.json'
                                    """
                                } catch (org.jenkinsci.plugins.workflow.steps.FlowInterruptedException e) {
                                    error "TIMEOUT ${e.toString()}"
                                } catch (Throwable e) {
