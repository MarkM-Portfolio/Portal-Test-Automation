...
pipeline {
    parameters {
        ...
        string(name: 'DX_UPDATE_CFV95_BINARY_AMI_NAME', defaultValue: 'dx-update-cf-v95_secondary_20220706-0423', description: 'Name of the AMI to build portal off of. For eg. dx-update-cf-v95_20220621-0817',  trim: false)
                ...
                script {
                    ...
                    dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                        ...
                        */
                        ...
                        env.TF_VAR_CF_AMI_ID = "${DX_UPDATE_CFV95_BINARY_AMI_NAME}"
                        if (!params.DX_UPDATE_CFV95_BINARY_AMI_NAME) {
                            env.TF_VAR_CF_AMI_ID = "dx-binary-cf-v95_*"
                        }
                    ...
                    withCredentials([
                        ...
                        dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                            configFileProvider([
                                ...
                                sh  """
                                    ssh -i test-automation-deployments-primary.pem -o StrictHostKeyChecking=no root@${env.INSTANCE_IP} \
            ...
            steps {
                dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                    sh """
                        ...
                        centos@${env.SECONDARY_INSTANCE_IP} '(cd /tmp/dx-onpremise/scripts && sudo sh 01-setup-prereqs-secondary.sh ${env.SECONDARY_PRIVATE_IP} ${env.SECONDARY_INSTANCE_NAME}${env.DOMAIN_SUFFIX} ${env.PRIVATE_IP} ${env.INSTANCE_NAME}${env.DOMAIN_SUFFIX} ${env.DMGR_HOST_DEFAULT})'
                    """
                }
            }
        }
        ...
        */
        stage('Copy Profile.zip into additional node') {
            steps {
                dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                    sh """
                        sed -i "s|DMGR_HOST|${env.DMGR_HOST_DEFAULT}|g;" ${workspace}/dx-cluster-onpremise/scripts/cluster/properties/additional-node/AddSecondaryNode.properties
                        sed -i "s|DMGR_HOST|${env.DMGR_HOST_DEFAULT}|g;" ${workspace}/dx-cluster-onpremise/scripts/cluster/properties/additional-node/FederateNodeRemote.properties
                        scp -i test-automation-deployments.pem -o StrictHostKeyChecking=no -r ${workspace}/dx-cluster-onpremise/scripts/cluster/properties/additional-node/AddSecondaryNode.properties centos@${env.SECONDARY_INSTANCE_IP}:/tmp/dx-onpremise/scripts/cluster/properties/additional-node/
                        scp -i test-automation-deployments.pem -o StrictHostKeyChecking=no -r ${workspace}/dx-cluster-onpremise/scripts/cluster/properties/additional-node/FederateNodeRemote.properties centos@${env.SECONDARY_INSTANCE_IP}:/tmp/dx-onpremise/scripts/cluster/properties/additional-node/
                    """

                    sh """
                        scp -i test-automation-deployments-primary.pem -o StrictHostKeyChecking=no root@${env.INSTANCE_IP}:/tmp/dx-onpremise/db2home.tar.gz .
                        ...
                        scp -i test-automation-deployments-primary.pem -o StrictHostKeyChecking=no root@${env.INSTANCE_IP}:/opt/IBM/WebSphere/PortalServer/profileTemplates/profileTemplates.zip .
        ...
        stage('Create Addtional node') {
            steps {
                dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                    ...
                    sh """
                        ssh -i test-automation-deployments.pem -o StrictHostKeyChecking=no centos@${env.SECONDARY_INSTANCE_IP} \
                        '(cd /tmp/dx-onpremise/scripts && sudo sh cluster/add-additional-node.sh ${env.DMGR_HOST_DEFAULT} ${env.INSTANCE_IP} ${env.SECONDARY_INSTANCE_HOSTNAME} ${env.DX_USERNAME} ${env.DX_PASSWORD} ${env.SECONDARY_INSTANCE_NAME} ${env.DOMAIN_SUFFIX} ${params.PERSONALIZED_DX_CORE_PATH} ${params.CONTEXT_ROOT_PATH} ${params.DX_CORE_HOME_PATH})'
                    """
                }
            }
        }
        ...
        stage('DB variables scope change') {
            steps {
                dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                    ...
                    sh """
                        scp -i test-automation-deployments-primary.pem -o StrictHostKeyChecking=no root@${env.INSTANCE_IP}:/opt/IBM/WebSphere/wp_profile/ConfigEngine/properties/wkplc_dbtype.properties .
                        scp -i test-automation-deployments-primary.pem -o StrictHostKeyChecking=no root@${env.INSTANCE_IP}:/opt/IBM/WebSphere/wp_profile/ConfigEngine/properties/wkplc_dbdomain.properties .
