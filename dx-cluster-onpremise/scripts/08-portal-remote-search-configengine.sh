...
sudo /opt/IBM/WebSphere/wp_profile/ConfigEngine/./ConfigEngine.sh configure-portal-for-remote-search -DWasPassword=${WAS_PASSWORD} -DPortalAdminPwd=${DX_PASSWORD} -Dremote.search.host.name=${REMOTE_SEARCH_INSTANCE_IP} -Dremote.search.iiop.url=iiop://${REMOTE_SEARCH_INSTANCE_IP}:2809
