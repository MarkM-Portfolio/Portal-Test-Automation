...
pipeline {
    ...
    stages {
                        ...
                        dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                            ...
                            */
                            ...
                            if(SELECTED_CORE_IMAGE_TAG == 'develop' || SELECTED_CORE_IMAGE_TAG == 'release') {
                                ...
                                tarFile  = sh (script: "${workspace}/dx-cluster-onpremise/scripts/get_latest_image.sh builds rohan_${SELECTED_CORE_IMAGE_TAG} ${url}", returnStdout: true)
                                ...
                                if(CF_BUILD_NUMBER <= 19) {
                                    SELECTED_CORE_IMAGE_TAG  = sh (script: "${workspace}/dx-cluster-onpremise/scripts/get_latest_image.sh ${CORE_MASTER_VERSION}", returnStdout: true)
                                }
        ...
        stage('Retrieve IP for remote search') {
        ...
        */
            ...
            steps {
                    dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                        ...
                        sh  """
                            ...
                            DX_CORE_BUILD_VERSION="${SELECTED_CORE_IMAGE_TAG}" DX_IMAGE="${SELECTED_CORE_IMAGE_TAG}" CF_VERSION="${CF_BUILD_NUMBER}" ${workspace}/dx-cluster-onpremise/scripts/${env.retrieveFilesFrom}
                        """
                    }
            }
        }
                ...
                withCredentials([
                    ...
                    dir("${workspace}/dx-cluster-onpremise/terraform/ec2-dx-onpremise-launch") {
                        ...
                        sh  """
                            ...
                            scp -i test-automation-deployments.pem -o StrictHostKeyChecking=no -r ${workspace}/dx-cluster-onpremise/scripts/helpers/UpdateRemoteSearch.xml centos@${REMOTE_SEARCH_HOST_NAME}:/tmp/dx-onpremise/scripts/helpers/UpdateRemoteSearch.xml
                        """
        ...
        stage('Retrieve IP for secondary node') {
